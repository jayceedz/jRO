...
int map_readallmaps (void)
{
	...
	if( enable_grf )
		...
		i = 1;
#else
		if (battle_config.renewal_map_cache)
			i = 0;
		else
			i = 1;
#endif
	...
	for (int i = 0; i < map_num; i++) {
		...
		if( enable_grf ){
			...
			if( map_cache_buffer[2] != NULL ){
				success = map_readfromcache( mapdata, map_cache_buffer[2], map_cache_decode_buffer ) != 0;
			}
	...
	if( !enable_grf ) {
		...
		if( map_cache_buffer[2] != NULL ){
			aFree(map_cache_buffer[2]);
		}
		aFree(map_cache_buffer[1]);
		if (map_cache_buffer[0] != NULL)
