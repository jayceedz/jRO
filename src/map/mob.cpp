...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	if( !rebirth ) { // Only trigger event on final kill
		...
		if( md->npc_event[0] && !md->state.npc_killmonster ) {
			...
			pc_setreg(mvp_sd, add_str("@killedgid_m"), md->mob_id);
...
#else
	if (battle_config.renewal_mob_db)
		fi = 0;
	else
		fi = 1;
#endif
	for (fi; fi < ARRAYLENGTH(mob_db_name); ++fi) {
...
static int mob_read_sqlskilldb(void)
{
...
#else
	if (battle_config.renewal_mob_skill_db)
		fi = 0;
	else
		fi = 1;
#endif
	...
	for( fi; fi < ARRAYLENGTH(mob_skill_db_name); ++fi ) {
		uint32 lines = 0, count = 0;
...
static void mob_load(void)
{
	...
	if( db_use_sqldbs )
		...
		for(int i = 0; i < ARRAYLENGTH(dbsubpath); i++){
			...
			int n3 = strlen(db_path)+strlen(DBPATHRE)+strlen(dbsubpath[0])+1;
	...
	if (battle_config.renewal_mob_db)
		if (!o) {
			sv_readdb(dbsubpath3, "mob_db.txt", ',', 31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP, 31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP, -1, &mob_readdb_sub, silent);
			o = true;
		}
#endif
	...
	for(int i = 0; i < ARRAYLENGTH(dbsubpath); i++){	
		...
		if( !db_use_sqldbs )
			if (!g) {
				if (battle_config.renewal_mob_skill_db)
					mob_readskilldb(dbsubpath3, silent);
				g = true;
			}
#endif
...
#ifndef RENEWAL
		if (!s) {
			if (battle_config.renewal_mob_db_full) {
				sv_readdb(dbsubpath3, "mob_race2_db.txt", ',', 2, MAX_RACE2_MOBS, -1, &mob_readdb_race2, silent);
				sv_readdb(dbsubpath3, "mob_random_db.txt", ',', 4, 4, -1, &mob_readdb_group, silent);
				sv_readdb(dbsubpath3, "mob_branch.txt", ',', 4, 4, -1, &mob_readdb_group, silent);
				sv_readdb(dbsubpath3, "mob_poring.txt", ',', 4, 4, -1, &mob_readdb_group, silent);
				sv_readdb(dbsubpath3, "mob_boss.txt", ',', 4, 4, -1, &mob_readdb_group, silent);
			}
			else {
				if (battle_config.renewal_mob_race2_db)
					sv_readdb(dbsubpath3, "mob_race2_db.txt", ',', 2, MAX_RACE2_MOBS, -1, &mob_readdb_race2, silent);
				if (battle_config.renewal_mob_random_db)
					sv_readdb(dbsubpath3, "mob_random_db.txt", ',', 4, 4, -1, &mob_readdb_group, silent);
				if (battle_config.renewal_mob_branch)
					sv_readdb(dbsubpath3, "mob_branch.txt", ',', 4, 4, -1, &mob_readdb_group, silent);
				if (battle_config.renewal_mob_poring)
					sv_readdb(dbsubpath3, "mob_poring.txt", ',', 4, 4, -1, &mob_readdb_group, silent);
				if (battle_config.renewal_mob_boss)
					sv_readdb(dbsubpath3, "mob_boss.txt", ',', 4, 4, -1, &mob_readdb_group, silent);
			}
			if (battle_config.renewal_mob_drop)
				sv_readdb(dbsubpath3, "mob_drop.txt", ',', 3, 5, -1, &mob_readdb_drop, silent);
			s = true;
		}
#endif
